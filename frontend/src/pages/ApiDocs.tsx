...
const endpoints = [
  ["GET", "/api/health", "健康检查（含 ComfyManager 与各云端提供商配置状态）"],
  ["GET", "/api/models", "从 ComfyManager 读取 ComfyUI 工位模型与工作流（云端提供商不用这个）"],
  ["GET", "/api/qwen/models", "千问云静态目录（视铸生成服务自带，不经过 ComfyManager）"],
  ["GET", "/api/cloud/models", "Meshy / Midjourney / Tripo / 火山引擎静态目录"],
  ["GET/PUT", "/api/settings", "ComfyManager 地址、各提供商 Key、工位工具、成品目录"],
...
export function ApiDocs() {
  ...
  return (
    <section className="mx-auto max-w-5xl px-8 py-10">
      <PageHead kicker="Open API" title="本地调用" desc={`生成接口和 MCP 在 18787。请求体可带 engine=comfyui / qwen / meshy / midjourney / tripo / volcengine。ComfyUI 部署在 ComfyManager ${mgr}；千问走 DashScope；Meshy / Tripo 走各自官方 OpenAPI；火山引擎生图/视频走方舟，生音乐走 HMAC OpenAPI。`} />
