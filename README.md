...
3. 选工位后点「加入」，或在下方工位卡片里从库选择后加入。同一工位可加入多份。
...
- **粘贴 JSON**：在上传按钮下方填写文件名并粘贴内容，效果和上传文件相同，写入同一目录。同名会覆盖。
- **删除**：库列表每行可删，或勾选后「删除选中」。会删掉磁盘上的 json，并从工位里拿掉引用。
...
| **本工位工作流** | 可多份，从库里的 json 加入。勾选「生效」会出现在视铸下拉里；「默认」是未指定时用的那份。 |
...
| **自定义 HTTP 请求体** | 仅 `http` 模式。工位上编辑请求体 JSON，不是往库里存 Comfy 图。 |
